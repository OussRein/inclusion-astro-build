...
export interface AppPortfolioConfig {
  isEnabled: boolean;
  postsPerPage: number;
  isRelatedPostsEnabled: boolean;
  relatedPostsCount: number;
...
const getAppPortfolio = (config: Config) => {
  const _default = {
    isEnabled: false,
    postsPerPage: 6,
    isRelatedPostsEnabled: false,
  ...
  return merge({}, _default, config?.apps?.portfolio ?? {}) as AppPortfolioConfig;
};
...
export default (config: Config) => ({
  ...
  APP_PORTFOLIO: getAppPortfolio(config),
  UI: getUI(config),
